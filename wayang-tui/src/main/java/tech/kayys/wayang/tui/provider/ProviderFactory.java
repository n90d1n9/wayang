...
public final class ProviderFactory {
    ...
    public static Provider create(Config config, Config.Profile profile) {
        ...
        return switch (pc.type) {
            case "anthropic" -> new AnthropicProvider(pc.baseUrl, Config.resolveApiKey(pc), pc.apiKeyEnv, profile.model);
            case "openai" -> new OpenAiProvider(pc.baseUrl, Config.resolveApiKey(pc), pc.apiKeyEnv, profile.model);
            case "demo" -> new DemoProvider();
            case "engine" -> new AgentEngineProvider(pc.baseUrl, "tdd");
