 ...
 */
final class WayangA2uiDecodeValues {
    ...
    static int nonNegativeInt(Object value, int fallback) {
        if (value == null) {
            return fallback;
        }
    ...
    static int clampedNonNegativeInt(Object value, int fallback) {
        long count = nonNegativeLong(value, fallback);
        return count > Integer.MAX_VALUE ? Integer.MAX_VALUE : Math.toIntExact(count);
    }

    static long nonNegativeLong(Object value, long fallback) {
        if (value == null) {
            return fallback;
        }
        if (value instanceof Number number) {
    ...
    private WayangA2uiDecodeValues() {
    }
}
