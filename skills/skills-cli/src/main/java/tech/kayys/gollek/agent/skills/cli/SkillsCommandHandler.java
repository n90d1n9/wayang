 ...
 * {@link GitSkillLoader} and {@link ExternalSkillStore} for actual work.
...
public class SkillsCommandHandler {
    ...
    public SkillsCommandHandler(GitSkillLoader gitLoader, LocalSkillLoader localLoader, ExternalSkillStore store) {
    ...
    public void add(String source, String skillFilter) {
        ...
        if (java.nio.file.Files.isDirectory(java.nio.file.Path.of(source))) {
            result = localLoader.install(source, skillFilter);
        } else {
            result = gitLoader.install(source, skillFilter);
        }
    ...
    public void remove(String repoName) {
        ...
        boolean removed = gitLoader.remove(repoName);
        if (removed) {
    ...
    public void update(String repoName) {
        if (repoName != null && !repoName.isBlank()) {
            ...
            // Update all repos
            List<String> repos = gitLoader.listInstalledRepos();
            if (repos.isEmpty()) {
    ...
    private void updateSingleRepo(String repoName) {
        ...
        // Try local loader first (checks for .local_source), fallback to git loader
        SkillInstallResult result = localLoader.update(repoName);
        if (result.errors().stream().anyMatch(e -> e.contains("Not a locally installed repository"))) {
            result = gitLoader.update(repoName);
        }
