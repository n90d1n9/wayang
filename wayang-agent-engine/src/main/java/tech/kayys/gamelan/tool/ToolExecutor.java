...
public class ToolExecutor {
    ...
    @Inject tech.kayys.gamelan.governance.PermissionManager permissionManager;
    ...
    public ToolResult execute(ToolCall call, String sessionId) {
        ...
        // ── Section XI: Permission gate check ──────────────────────────────
        String description = describeCall(call);
        if (!permissionManager.isPermitted(sessionId, call, description)) {
            log.info("[permission] tool '{}' denied by user or policy", call.name());
            return ToolResult.failure(call.name(),
                    "Execution denied by permission manager or human-in-the-loop gate.");
        }
