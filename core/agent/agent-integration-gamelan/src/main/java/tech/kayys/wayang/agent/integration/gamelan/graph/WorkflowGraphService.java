...
import tech.kayys.wayang.agent.integration.gamelan.AgentGamelanService;
    ...
    public Uni<String> recordExecutionRun(String agentId, RunResponse runResponse) {
        ...
        return Uni.createFrom().item(() -> {
            ...
            Node executionRunNode = Node.builder()
                ...
                .property("startTime", runResponse.getStartedAt() != null
                    ? runResponse.getStartedAt().toString() : Instant.now().toString())
                .property("endTime", runResponse.getCompletedAt() != null
                    ? runResponse.getCompletedAt().toString() : null)
                .property("durationMs", runResponse.getDurationMs() != null
                    ? runResponse.getDurationMs() : 0L)
                .property("nodesExecuted", runResponse.getNodesExecuted() != null
                    ? runResponse.getNodesExecuted() : 0)
                .property("nodesTotal", runResponse.getNodesTotal() != null
                    ? runResponse.getNodesTotal() : 0)
